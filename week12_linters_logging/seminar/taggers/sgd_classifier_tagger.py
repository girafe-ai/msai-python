...
class SGDClassifierTestTagger(BaseChoiceTagger):
    ...
    def __init__(self, tags: list[str] = None):
        ...
        self.twenty_train = fetch_20newsgroups(
            subset='train', categories=self.tags, shuffle=True, random_state=42,
        )
...
class SGDClassifierTagger(BaseTagger):
    def __init__(self, classification_threshold=0.2, filename_with_weights='taggers/model_weights.pik'):
            ...
            [
                ...
                (
                    'clf_svm', SGDClassifier(
                        learning_rate='adaptive', eta0=0.1, loss='modified_huber', penalty='elasticnet',
        ...
        try:
            ...
            print(
                f'Model accuracy:{np.mean(predictions == twenty_test.target):.2f}',
            )
    ...
    def get_tags(self, texts: list[str]) -> list[list[str]]:
        ...
        for text in texts:
            ...
            sorted_scores = [(
                predictions[i], predictions.index(
                    predictions[i],
                ),
            ) for i in range(len(predictions))]
            sorted_scores = sorted(
                sorted_scores, reverse=True, key=lambda x: x[0],
            )
            valid_predictions = []
